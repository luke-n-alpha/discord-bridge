...
principles:
  ...
  - tdd_first: write failing tests before implementation
  - e2e_validation: finalize changes with end-to-end scenarios
  - no_hardcoding: move paths/models/lang/schedule/SMTP to config
  - reuse_before_new: analyze existing modules before new code
workflows:
  ...
  task_specs:
    ...
    filename: YYYYMMDD-%2dn-{topic}.md
  work_logs:
    path: work-logs/luke/discord-summarizer/02.doing
    filename: YYYYMMDD-%2dn-{topic}.md
    rule: use a two-digit sequence with trailing "n" per day
doc_sync:
